...
#' extreme_scores(): Calcuting frequency each person fell above or below a given crossword time or zscore
#'
#' df: dataframe
...
# Tibble for time_z > 2
extreme_scores(crossword_scores, time_var = "time_z", sign = ">", threshold = 2)

# Tibble for time_z < -1
extreme_scores(crossword_scores, time_var = "time_z", sign = "<", threshold = -1)

# Tibble for time <= 60
extreme_scores(crossword_scores, time_var = "time", sign = "<=", threshold = 60)

# Tibble for rank <= 3
extreme_scores(crossword_scores, time_var = "rank", sign = "<=", threshold = 3)
